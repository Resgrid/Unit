...
export default function RouteViewScreen() {
  ...
  const assignedUnitName = activePlan?.UnitId != null ? unitMap[activePlan.UnitId] || (String(activePlan.UnitId) === String(activeUnitId) ? (activeUnit?.Name ?? '') : '') : null;
  ...
  if (error || !activePlan) {
    return (
      <View className="size-full flex-1 bg-gray-50 dark:bg-gray-900">
        <FocusAwareStatusBar />
        <ZeroState heading={t('routes.no_routes')} description={error || t('routes.no_routes_description')} isError={!!error} />
      </View>
    );
  }
    ...
    return (
      <React.Fragment key={stop.RouteStopId}>
        <HStack className="items-start gap-3 py-3">
          {/* Order badge */}
          <View style={[styles.stopBadge, { backgroundColor: markerColor }]}>
          ...
          <VStack className="flex-1 gap-1">
            ...
            {stop.Notes ? <Text className="text-xs italic text-gray-500 dark:text-gray-400">{stop.Notes}</Text> : null}

            {stop.ContactId ? (
              <Pressable onPress={() => router.push(`/routes/stop/contact?stopId=${stop.RouteStopId}` as any)}>
                <HStack className="mt-1 items-center gap-1.5">
                  <Icon as={User} size="xs" className="text-blue-500" />
                  <Text className="text-xs font-medium text-blue-600 dark:text-blue-400">{t('routes.view_contact')}</Text>
                </HStack>
              </Pressable>
            ) : null}
          </VStack>
    ...
    <View className="size-full flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <Box className="px-4 pt-4">
          ...
          <Box className="rounded-xl bg-white p-5 shadow-sm dark:bg-gray-800" style={{ borderLeftWidth: 4, borderLeftColor: markerColor }}>
            ...
            {/* Description */}
            {activePlan.Description ? <Text className="mt-2 text-sm text-gray-600 dark:text-gray-400">{activePlan.Description}</Text> : null}

            {/* Assigned unit */}
            <HStack className="mt-2 items-center gap-1.5">
              <Icon as={Truck} size="xs" className={assignedUnitName ? 'text-blue-500' : 'text-gray-400'} />
              <Text className={`text-sm font-medium ${assignedUnitName ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`}>{assignedUnitName || t('routes.unassigned')}</Text>
            </HStack>
            ...
            <HStack className="mt-4 gap-3">
              ...
              {(activePlan.EstimatedDistanceMeters ?? 0) > 0 ? (
                <VStack className="items-center rounded-lg bg-gray-50 px-4 py-2 dark:bg-gray-700">
                  <Text className="text-lg font-bold text-blue-600 dark:text-blue-400">{formatDistance(activePlan.EstimatedDistanceMeters!)}</Text>
              ...
              {(activePlan.EstimatedDurationSeconds ?? 0) > 0 ? (
                <VStack className="items-center rounded-lg bg-gray-50 px-4 py-2 dark:bg-gray-700">
                  <Text className="text-lg font-bold text-blue-600 dark:text-blue-400">{formatDuration(activePlan.EstimatedDurationSeconds!)}</Text>
          ...
          <Box className="mt-4 rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
            ...
            {sortedStops.length > 0 ? <VStack>{sortedStops.map((stop, index) => renderStopRow(stop, index))}</VStack> : <Text className="py-4 text-center text-gray-500 dark:text-gray-400">{t('routes.no_stops')}</Text>}
          </Box>
        </Box>
      </ScrollView>
      ...
      {canStart ? (
        <Box className="border-t border-gray-200 bg-white px-4 py-4 dark:border-gray-700 dark:bg-gray-800">
          <Button size="xl" className="w-full bg-green-600" onPress={handleStartRoute} isDisabled={isLoading} testID="start-route-button">
