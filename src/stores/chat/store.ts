...
/**
 * Unit-app chat identity: the active unit *is* the chat identity ("Engine 6").
 * The core store keeps activeUnitId as a string; the Chat API expects a numeric
 * AsUnitId. Returns undefined when no unit is selected so the caller falls back
 * to the signed-in user's identity.
 */
function activeUnitIdNumber(): number | undefined {
  const id = useCoreStore.getState().activeUnitId;
 ...
 * The active unit is this app's chat identity, so it wins over the signed-in user.
 */
function currentDisplayName(): string | null {
  const unitName = useCoreStore.getState().activeUnit?.Name;
  ...
  persist(
    (set, get) => ({
      ...
      fetchChannels: async (activeUnitId?: number) => {
        // The Unit app scopes the channel list to the active unit ("Engine 6").
        const unitId = activeUnitId ?? activeUnitIdNumber();
      ...
      sendMessage: async (args: SendMessageArgs) => {
        ...
        // Unit app posts as the active unit ("Engine 6") unless a caller overrides.
        const asUnitId = args.asUnitId ?? activeUnitIdNumber();
      ...
      markChannelRead: async (channelId: string) => {
        ...
        // Hub signature: MarkRead(channelId, seq, asUnitId). SignalR binds hub arguments
        // positionally and rejects an invocation that supplies fewer than the method
        // declares, so asUnitId is always sent — null when there is no active unit.
        void safeInvoke('MarkRead', channelId, seq, asUnitId ?? null);
        try {
      ...
      joinChannel: async (channelId: string) => {
        // Join as the active unit so hub-side membership resolves to the unit.
        const asUnitId = activeUnitIdNumber();
        // Hub signature: JoinChannel(channelId, asUnitId). Omitting the optional argument
        // makes SignalR reject the invocation, which leaves the connection outside the
        // channel group and the channel permanently silent.
        await safeInvoke('JoinChannel', channelId, asUnitId ?? null);
      },
      ...
      sendTyping: (channelId: string, isTyping: boolean) => {
        ...
        const asUnitId = activeUnitIdNumber();
        // Hub signature: Typing(channelId, displayName, isTyping, asUnitId). The previous
        // call put isTyping in the displayName slot and dropped two arguments, so the hub
        // rejected every typing signal.
        void safeInvoke('Typing', channelId, currentDisplayName(), isTyping, asUnitId ?? null);
      },
