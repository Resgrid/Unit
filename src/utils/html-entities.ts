...
// only when there are no actual tags in the value.
export const looksHtmlEncoded = (value: string): boolean => !/<[a-z!/]/i.test(value) && /&(lt|#0*60|#x0*3c);/i.test(value);

/**
 * Decodes HTML entities only when the value looks entity-encoded
 * (no real tags, but encoded tag markers present). Otherwise the
