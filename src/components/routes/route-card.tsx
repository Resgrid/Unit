...
export const RouteCard: React.FC<RouteCardProps> = ({ route, isActive = false, unitName, isMyUnit = false }) => {
  ...
  return (
    <Box className={bgClass} style={{ borderLeftWidth: 4, borderLeftColor: borderColor }}>
      <HStack className="items-start justify-between">
        <VStack className="flex-1">
          ...
          <HStack className="mt-2 items-center gap-1.5">
            ...
            <Text className={`text-xs font-medium ${isMyUnit ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`}>{unitName || t('routes.unassigned')}</Text>
          </HStack>

          <HStack className="mt-2 gap-4">
            <HStack className="items-center gap-1.5">
              <Icon as={MapPin} size="xs" className="text-gray-400" />
              <Text className="text-sm text-gray-500 dark:text-gray-400">{t('routes.stop_count', { count: route.StopsCount ?? 0 })}</Text>
            </HStack>

            {(route.EstimatedDistanceMeters ?? 0) > 0 ? (
              <HStack className="items-center gap-1.5">
                <Icon as={Navigation} size="xs" className="text-gray-400" />
                <Text className="text-sm text-gray-500 dark:text-gray-400">{formatDistance(route.EstimatedDistanceMeters!)}</Text>
              </HStack>
            ) : null}
            ...
            {(route.EstimatedDurationSeconds ?? 0) > 0 ? (
              <HStack className="items-center gap-1.5">
                ...
                <Text className="text-sm text-gray-500 dark:text-gray-400">{formatDuration(route.EstimatedDurationSeconds!)}</Text>
              </HStack>
            ) : null}
          </HStack>
        </VStack>
